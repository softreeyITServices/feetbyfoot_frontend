      ...
      complete: async (results) => {
        try {
          ...
          for (const handle of handles) {
            ...
            // Parse Base Images
            const baseImageNames = firstRow.BaseImageUrls ? firstRow.BaseImageUrls.split(",").map((s: string) => s.trim()) : [];
            ...
            const sizes: ProductSize[] = await Promise.all(
              groupRows.map(async (row) => {
                const specificImageNames = row.SpecificImageUrls ? row.SpecificImageUrls.split(",").map((s: string) => s.trim()) : [];
                ...
                return {
                  ...
                  title: row.SpecificTitle || "",
                  description: row.SpecificDescription || "",
            ...
            const payload: CreateProductPayload = {
              ...
              categoryTypeId: firstRow.CategoryType_ID || "",
              categoryTypeIds: firstRow.CategoryType_ID ? [firstRow.CategoryType_ID] : [],
              color: colors[0] || "",
              colors,
              sizes: validSizes,
              imageUrls: validBaseImages,
              gender: firstRow.Gender ? firstRow.Gender.split(",").map((s: string) => s.trim()) : [],
              tags: firstRow.Tags ? firstRow.Tags.split(",").map((s: string) => s.trim()) : [],
              isActive: firstRow.IsActive ? firstRow.IsActive.toLowerCase() === "true" || firstRow.IsActive === "1" : true,
          ...
          if (error.data?.message && Array.isArray(error.data.message)) {
            ...
            toast.error(error.data?.message || error.message || "Failed to process CSV");
          }
        } finally {
          setLoading(false);
          setProgress("");
          ...
          <div className="space-y-2">
            ...
            <div className="relative border-2 border-dashed border-neutral-200 rounded-xl p-4 text-center hover:bg-neutral-50 transition-colors">
              ...
              {csvFile ? (
                ...
                <p className="text-sm text-neutral-500">Click or drag CSV file here</p>
              )}
            </div>
            <p className="text-xs text-neutral-500">
              Ensure it matches the expected template format.
            </p>
          </div>
